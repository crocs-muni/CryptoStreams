...
public:
    ...
    block_test_case(const std::string &&algorithm,
                    const std::size_t round,
                    const bool load_iv = false)
     ...
     * test-resources/block/{function-name}/test-vectors-{round-number}.txt) {key in hex} {plaintext
     * in hex} {ciphertext in hex} {initialization-vector in hex} # Only in case _load_iv is true
     * {newline}
     * {key in hex}
     * {plaintext in hex}
     * {ciphertext in hex}
