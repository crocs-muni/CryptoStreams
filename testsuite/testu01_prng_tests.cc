...
TEST(LCG, test_include_seed) {
    ...
    for (auto k = 1; k <= 10000; k++) {
        ...
        for (int i = 0; i < 5; i++) {
            seed |= static_cast<int64_t >(data[i]) << (8*i);
        }

        int64_t output1 = *reinterpret_cast<uint64_t*>(data.data() + 5);
        int64_t output2 = *reinterpret_cast<uint64_t*>(data.data() + 13);
...
TEST(MRG, basic_test) {
    ...
    std::vector<uint8_t> data((viable_bytes * 2) + 8);
    ...
    for (auto k = 1; k <= 30; k++) {
        ...
        for (auto l = 0; l < viable_bytes; l++) {
            s1 |= static_cast<long>(data[j++]) << (8 * l);
        }

        long s2 = 0;

        for (auto l = 0; l < viable_bytes; l++) {
            s2 |= static_cast<long>(data[j++]) << (8 * l);
        }

        uint64_t output = *reinterpret_cast<uint64_t*>(data.data() + j);
