...
#ifndef MOUSTIQUE_SSYN
#define MOUSTIQUE_SSYN
 ...
 */
#define MOUSTIQUE_NAME "MOUSTIQUE"    /* [edit] */ 
 ...
 * following code:
 *
 * for (i = 0; MOUSTIQUE_KEYSIZE(i) <= MOUSTIQUE_MAXKEYSIZE; ++i)
 *   {
 *     keysize = MOUSTIQUE_KEYSIZE(i);
 *
 *     ...
 *   }
 *
...
#define MOUSTIQUE_MAXKEYSIZE 96       
#define MOUSTIQUE_KEYSIZE(i) (96 + (i)*1)

#define MOUSTIQUE_MAXIVSIZE 104          
#define MOUSTIQUE_IVSIZE(i) (0 + (i)*8)  
 ...
 */
#define MOUSTIQUE_SYNCLENGTH 14                  /* actually 105 bits */
...
/* 
 * MOUSTIQUE_ctx is a structure storing the (expanded) key and the internal state of the
 * cipher.
 */
...
typedef struct
{
  ...
  u8 *C_0;           /* a[1...96]    is CCSR_0[1...96] */
  u8 *C_1;           /* a[97...104]  is CCSR_1[89...96] */
  u8 *C_2;           /* a[105...108] is CCSR_2[93...96] */
  u8 *C_3;           /* a[109...112] is CCSR_3[93...96] */
  u8 *C_4;           /* a[113...114] is CCSR_4[95...96] */
  u8 *C_5;           /* a[115...116] is CCSR_5[95...96] */
  u8 *C_6;           /* a[117...118] is CCSR_6[95...96] */
  u8 *C_7;           /* a[119...120] is CCSR_7[95...96] */
  u8 *C_8;           /* a[121] is CCSR_8[96] */
  u8 *C_9;           /* a[122] is CCSR_9[96] */
  u8 *C10;           /* a[123] is CCSR_10[96] */
  u8 *C11;           /* a[124] is CCSR_11[96] */
  u8 *C12;           /* a[125] is CCSR_12[96] */
  u8 *C13;           /* a[126] is CCSR_13[96] */
  u8 *C14;           /* a[127] is CCSR_14[96] */
  u8 *C15;           /* a[128] is CCSR_15[96] */
} MOUSTIQUE_ctx;     /* key and internal state */

/* ------------------------------------------------------------------------- */
class ECRYPT_Moustique: public estream_interface
{
public:
	/* Mandatory functions */
	 ...
	 * please undef the MOUSTIQUE_USES_DEFAULT_ALL_IN_ONE flag.
	 */
	#define MOUSTIQUE_USES_DEFAULT_ALL_IN_ONE        /* [edit] */

	void MOUSTIQUE_encrypt_packet(
	  MOUSTIQUE_ctx* ctx,
	  const u8* iv,
	  const u8* plaintext, 
	  u8* ciphertext, 
	 ...
	 * for each instance of MOUSTIQUE_ctx 
	 */
	 ...
	 * MOUSTIQUE_USES_DEFAULT_BLOCK_MACROS flag and implement the functions
	 * declared below.
	 */

	#define MOUSTIQUE_USES_DEFAULT_BLOCK_MACROS      /* [edit] */
	#ifdef MOUSTIQUE_USES_DEFAULT_BLOCK_MACROS

	#define MOUSTIQUE_encrypt_blocks(ctx, previous, plaintext, ciphertext, blocks)\
	  ECRYPT_encrypt_bytes(ctx, previous, plaintext, ciphertext,               \
		(blocks) * MOUSTIQUE_SYNCLENGTH)

	#define MOUSTIQUE_decrypt_blocks(ctx, previous, ciphertext, plaintext, blocks)\
	  ECRYPT_decrypt_bytes(ctx, previous, ciphertext, plaintext,               \
		(blocks) * MOUSTIQUE_SYNCLENGTH)

	#else

	void MOUSTIQUE_encrypt_blocks(
	  const MOUSTIQUE_ctx* ctx, 
	  const u8* previous,
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void MOUSTIQUE_decrypt_blocks(
	  const MOUSTIQUE_ctx* ctx, 
	  const u8* previous,
	  const u8* ciphertext, 
	  u8* plaintext, 
