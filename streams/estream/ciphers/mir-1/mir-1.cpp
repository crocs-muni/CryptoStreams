...
/* ------------------------------------------------------------------------- */
void ECRYPT_Mir::ECRYPT_init() {}

/* ------------------------------------------------------------------------- */
void ECRYPT_Mir::ECRYPT_keysetup(const u8* key,
...
/* ------------------------------------------------------------------------- */
void ECRYPT_Mir::ECRYPT_ivsetup(const u8* IV) {
    MIR1_ctx* ctx = &_ctx;
    int i;
    ...
    {Mir1_STATE_UPDATE} { Mir1_STATE_UPDATE }
}

/* ------------------------------------------------------------------------- */
// NOTE: msglen MUST BE divisible by 8
void ECRYPT_Mir::ECRYPT_encrypt_bytes(const u8* plaintext,
                                      u8* ciphertext,
                                      u32 msglen) /* Message length in bytes. */
{
    ...
    for (i = 0; i < k; i += 8) {
        Mir1_STATE_UPDATE U64TO8_LITTLE(ciphertext + i, U8TO64_LITTLE(plaintext + i) ^ ctx->b.v);
    }

    if (k < msglen) {
        Mir1_STATE_UPDATE for (i = k; i < msglen; ++i) {
...
/* ------------------------------------------------------------------------- */
void ECRYPT_Mir::ECRYPT_decrypt_bytes(const u8* ciphertext,
                                      u8* plaintext,
                                      u32 msglen) /* Message length in bytes. */

{
    ...
    for (i = 0; i < k; i += 8) {
        Mir1_STATE_UPDATE U64TO8_LITTLE(plaintext + i, U8TO64_LITTLE(ciphertext + i) ^ ctx->b.v);
    }
...
/* ------------------------------------------------------------------------- */
void ECRYPT_Mir::MIR1_keystream_bytes(MIR1_ctx* ctx,
                                      u8* keystream,
                                      u32 length) /* Length of keystream in bytes. */
{
    ...
    for (i = 0; i < k; i += 8) {
        Mir1_STATE_UPDATE U64TO8_LITTLE(keystream + i, ctx->b.v);
    }

    if (k < length) {
        Mir1_STATE_UPDATE for (i = k; i < length; ++i) {
