...
void ECRYPT_Edon80::ECRYPT_keysetup(const u8* key, u32 keysize, u32 ivsize) {
    EDON80_ctx* ctx = &_ctx;
    ...
    ctx->NumberOfInternalStates = EDON80_MAXKEYSIZE;
    // ctx->NumberOfInternalStates=numRounds;
    ...
    for (m = 0; m<keysize>> 1; m++) {
        ...
        for (i = 0; i < 4; i++)
            for (j = 0; j < 4; j++) {
                ...
                ctx->Q[m + EDON80_MAXKEYSIZE / 2][i][j] = Q[index][i][j];
            }
    }
}

/* Here is the actual definition of ECRYPT_ivsetup */
void ECRYPT_Edon80::ECRYPT_ivsetup(const u8* iv) {
    EDON80_ctx* ctx = &_ctx;
    u32 i, j;                   /* Variables i and j are internal counters. */
    s32 k;                      /* Variable k is internal signed counter. */
    u8 Temp[EDON80_MAXKEYSIZE]; /* Temp is a vector of bytes that will temporarily
                                                                 hold the values of the vector
                                   InternalStates[] */
    ...
    for (i = 0; i < ctx->keysize; i++)
        Temp[i] = ctx->InternalState[i] = ctx->key[i];
...
void ECRYPT_Edon80::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    EDON80_ctx* ctx = &_ctx;
    u32 i, j; /* Variables i and j are internal counters. */
...
void ECRYPT_Edon80::ECRYPT_decrypt_bytes(const u8* ciphertext, u8* plaintext, u32 msglen) {
    EDON80_ctx* ctx = &_ctx;
    u32 i, j; /* Variables i and j are internal counters. */
    u8 X;     /* We will store produced byte from the keystream in X. */
...
void ECRYPT_Edon80::EDON80_keystream_bytes(EDON80_ctx* ctx, u8* keystream, u32 length) {
    u32 i, j; /* Variables i and j are internal counters. */
    u8 X;     /* We will store produced byte from the keystream in X. */
