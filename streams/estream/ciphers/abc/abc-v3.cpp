 ...
 *
 * Assumed variables:
 *  u32 z0, z1, z2, z3 - A primitive (LFSR) stat
...
#define ABC_PROCESS_WORD(offset)                                                                   \
    ABC_UPPER_PART(table);                                                                         \
    r = ABC_OUTPUT_WORD(table);                                                                    \
    U32TO8_LITTLE(output + offset, U8TO32_LITTLE(input + offset) ^ ABC_POSTFILTER(r))
 ...
 * above.
 */
void ECRYPT_ABC::ECRYPT_keysetup(const u8* key,
 ...
 * IV's.
 */
void ECRYPT_ABC::ECRYPT_ivsetup(const u8* iv) {
    ABC_ctx* ctx = &_ctx;
    /* apply IV to restored state */
    u32 d0 = (ctx->d0i ^ U8TO32_LITTLE(iv + 12)) & ABC_B_COEF_AND_MASK;
    u32 d1 = ((ctx->d1i ^ U8TO32_LITTLE(iv + 8)) & ABC_B_COEF_AND_MASK) | ABC_B_COEF_OR_MASK;
 ...
 */
void ECRYPT_ABC::ABC_process_bytes(int action, /* 0 = encrypt; 1 = decrypt; */
                                   void* ctxa,
                                   const u8* input,
                                   u8* output,
                                   u32 msglen) /* Message length in bytes. */
{
    ABC_ctx* ctx = (ABC_ctx*)ctxa;
 ...
 */
void ECRYPT_ABC::ABC_keystream_bytes(ABC_ctx* ctx,
                                     u8* keystream,
 ...
 */
void ECRYPT_ABC::ABC_process_blocks(int action, /* 0 = encrypt; 1 = decrypt; */
                                    ABC_ctx* ctx,
                                    const u8* input,
                                    u8* output,
                                    u32 blocks) /* Message length in blocks. */
{
    ...
    u32 i, r, s = 0;
#if defined(ABC_UNROLL_1)
    for (i = 0; i < blocks * ABC_BLOCKLENGTH; i += 4) {
        ABC_PROCESS_WORD(i);
    }
#elif defined(ABC_UNROLL_4)
    for (i = 0; i < blocks * ABC_BLOCKLENGTH; i += 16) {
        ABC_PROCESS_WORD(i);
        ABC_PROCESS_WORD(i + 4);
        ABC_PROCESS_WORD(i + 8);
        ABC_PROCESS_WORD(i + 12);
...
#elif defined(ABC_UNROLL_8)
    for (i = 0; i < blocks * ABC_BLOCKLENGTH; i += 32) {
        ABC_PROCESS_WORD(i);
        ABC_PROCESS_WORD(i + 4);
        ABC_PROCESS_WORD(i + 8);
        ABC_PROCESS_WORD(i + 12);
...
#elif defined(ABC_UNROLL_16)
    for (i = 0; i < blocks * ABC_BLOCKLENGTH; i += 64) {
        ABC_PROCESS_WORD(i);
        ABC_PROCESS_WORD(i + 4);
        ABC_PROCESS_WORD(i + 8);
        ABC_PROCESS_WORD(i + 12);
 ...
 */
void ECRYPT_ABC::ABC_keystream_blocks(ABC_ctx* ctx,
                                      u8* keystream,
...
void ECRYPT_ABC::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    // this implementation computes with msglen in bits instead of in bytes
    ABC_process_bytes(0, &_ctx, plaintext, ciphertext, 8 * msglen);
}
void ECRYPT_ABC::ECRYPT_decrypt_bytes(const u8* ciphertext, u8* plaintext, u32 msglen) {
    // this implementation computes with msglen in bits instead of in bytes
