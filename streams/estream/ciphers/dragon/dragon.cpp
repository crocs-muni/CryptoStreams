 ...
 * NLFSR.
 */
#define DRAGON_OFFSET(ctx, ith_element, state_size) ((ctx->nlfsr_offset + ith_element) & state_size)
...
void ECRYPT_Dragon::ECRYPT_init(void) {}
 ...
 * above.
 */
void ECRYPT_Dragon::ECRYPT_keysetup(const u8* key,
                                    ...
                                    u32 ivsize)  /* IV size in bits. */
{
    ...
    if (keysize == 128) {
        ...
        for (idx = 0; idx < 4; idx++) {
            ...
            ctx->nlfsr_word[0 + idx] = ctx->nlfsr_word[12 + idx] = ctx->nlfsr_word[20 + idx] =
                    key_word;
        }
        ...
        for (idx = 0; idx < 2; idx++) {
            ...
            ctx->nlfsr_word[4 + idx] = ctx->nlfsr_word[16 + idx] = ctx->nlfsr_word[28 + idx] =
                    key_word;

            key_word = U8TO32_BIG(key + idx * 4);
            ctx->nlfsr_word[6 + idx] = ctx->nlfsr_word[18 + idx] = ctx->nlfsr_word[30 + idx] =
                    key_word;
        }
    } else {
 ...
 * IV's.
 */
void ECRYPT_Dragon::ECRYPT_ivsetup(const u8* iv) {
    u32 a, b, c, d;
    u32 e = 0x00004472;
    u32 f = 0x61676F6E;
    u32 iv_word;
    ...
    */
    ...
    for (idx = 0; idx < _rounds; idx++) {
        a = DRAGON_NLFSR_WORD(ctx, 0) ^ DRAGON_NLFSR_WORD(ctx, 24) ^ DRAGON_NLFSR_WORD(ctx, 28);

        b = DRAGON_NLFSR_WORD(ctx, 1) ^ DRAGON_NLFSR_WORD(ctx, 25) ^ DRAGON_NLFSR_WORD(ctx, 29);

        c = DRAGON_NLFSR_WORD(ctx, 2) ^ DRAGON_NLFSR_WORD(ctx, 26) ^ DRAGON_NLFSR_WORD(ctx, 30);

        d = DRAGON_NLFSR_WORD(ctx, 3) ^ DRAGON_NLFSR_WORD(ctx, 27) ^ DRAGON_NLFSR_WORD(ctx, 31);
 ...
 */
void ECRYPT_Dragon::DRAGON_keystream_blocks(DRAGON_ctx* ctx, u8* keystream, u32 blocks) {
 ...
 */
void ECRYPT_Dragon::DRAGON_process_blocks(int action, /* 0 = encrypt; 1 = decrypt; */
                                          void* ctxa,
                                          const u8* input,
                                          u8* output,
                                          u32 blocks) {
    ...
    u32 a, b, c, d, e, f;
    DRAGON_ctx* ctx = (DRAGON_ctx*)ctxa;
 ...
 */
void ECRYPT_Dragon::DRAGON_keystream_bytes(DRAGON_ctx* ctx, u8* keystream, u32 length) {
    assert(ctx && keystream);
...
void ECRYPT_Dragon::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    DRAGON_process_bytes(0, &_ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Dragon::ECRYPT_decrypt_bytes(const u8* ciphertext, u8* plaintext, u32 msglen) {
    DRAGON_process_bytes(1, &_ctx, ciphertext, plaintext, msglen);
}
 ...
 */
void ECRYPT_Dragon::DRAGON_process_bytes(int action, /* 0 = encrypt; 1 = decrypt; */
                                         void* ctxa,
                                         const u8* input,
                                         u8* output,
                                         u32 msglen) {
    ...
    DRAGON_keystream_bytes(ctx, output, len);
