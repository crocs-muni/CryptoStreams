...
static const BYTE MACnibMix[32] =  { 3, 2, 1, 0, 7, 6, 5, 4,11,10, 9, 8,15,14,13,12,19,18,17,16,23,22,21,20,27,26,25,24,31,30,29,28};
...
***************/
 ...
 /* ZK-crypt INIT options */
	...
	DWORD runBank(ZKCRYPT_ctx* ctx, DWORD* ptrState, BYTE control, 
			int leftIndex, int rightIndex, int rotateNum, DWORD cipher, 
			DWORD maskLeftClockFB, DWORD maskRightClockFB);
	DWORD runSuperBank(ZKCRYPT_ctx* ctx, DWORD* ptrState, BYTE control,
			int leftIndex, int rightIndex, int rotateNum, DWORD cipher,
	...
	DWORD feedbackStorage(ZKCRYPT_ctx* ctx, DWORD in);
