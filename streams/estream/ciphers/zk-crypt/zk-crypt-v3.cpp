...
void ZKengine(ZKCRYPT_ctx* ctx, BOOL fZeroKeys) {
...
void ECRYPT_Zkcrypt::ECRYPT_init(void) {
...
void ZKCRYPT_init_X(ZKCRYPT_ctx* ctx, u32 options) {
    /*
...
void ECRYPT_Zkcrypt::ECRYPT_keysetup(const u8* key, u32 keysize, u32 ivsize) {
    ZKCRYPT_ctx* ctx = &_ctx;
...
void ECRYPT_Zkcrypt::ECRYPT_ivsetup(const u8* iv) {
    ZKCRYPT_ctx* ctx = &_ctx;
...
void ECRYPT_Zkcrypt::ZKCRYPT_process_bytes(int action, /* 0 = encrypt; 1 = decrypt; */
                                           void* ctxa,
                                           const u8* input,
                                           u8* output,
                                           u32 msglen)
...
{
    ZKCRYPT_ctx* ctx = (ZKCRYPT_ctx*)ctxa;
...
void ECRYPT_Zkcrypt::ZKCRYPT_keystream_bytes(ZKCRYPT_ctx* ctx, u8* keystream, u32 length)
...
DWORD setupZKengine(ZKCRYPT_ctx* ctx) {
    DWORD res = 0;
    int i;
    int keyBlockCount = 0;
    int ivBlockCount = 0;
...
DWORD cycleZKengine(ZKCRYPT_ctx* ctx, DWORD in)
/*	00B	&	00S	&	00F	&	1S	*/
{
    DWORD res = 0;
    DWORD lclMask = 0;
    static DWORD maskInversFeedBackClockBits =
            ~(CTRL_CLOCKS_FB_JUGG | CTRL_CLOCKS_FB_4TH | CTRL_CLOCKS_FB_DAS);
...
DWORD nLFSR_Bank(ZKCRYPT_ctx* ctx) {
    DWORD res = 0;
    ...
    if ((initOptions & ZK_OPTI_SUPER) > 0)
        tmpSprBank = runSuperBank(ctx,
                                  ...
                                  7,
                                  ctx->upCphWrd_5,
                                  CTRL_CLOCKS_FB_THSH08,
                                  CTRL_CLOCKS_FB_THSH18);

    /* 19C	& 24C	& 25S	*/
    lclCtrlBank = ctx->ctrlTopBank | CTRL_NLFSR_FB;
    tmpTopBank = runBank(ctx,
                         &(ctx->sttTopBank),
                         lclCtrlBank,
                         DEF_nLFSR_13,
                         DEF_nLFSR_19,
...
DWORD runBank(ZKCRYPT_ctx* ctx,
              ...
              int rotateNum,
              DWORD cipher,
              DWORD maskLeftClockFB,
              DWORD maskRightClockFB) {
    DWORD res = 0;
    DWORD image = 0;
    DWORD left, right;
...
DWORD runSuperBank(ZKCRYPT_ctx* ctx,
                   DWORD* ptrState,
                   BYTE control,
                   int leftIndex,
                   int rightIndex,
                   int rotateNum,
                   DWORD cipher,
                   DWORD maskLeftHashMask,
                   DWORD maskRightHashMask) {
    DWORD res = 0;
    DWORD image = 0;
    DWORD left, right;
    int lenLeft = nLFSR_Len[leftIndex];
...
DWORD topXor_N_Store(ZKCRYPT_ctx* ctx, DWORD in)
/*	33	&	11P	&	11S2	*/
{
    DWORD res;
...
DWORD intrXor_N_Store(ZKCRYPT_ctx* ctx, DWORD in)
/*	33	&	11P	&	11S2	*/
{
    DWORD res;
...
DWORD botXor_N_Store(ZKCRYPT_ctx* ctx, DWORD in)
/*	33	&	11P	&	11S2	*/
{
    DWORD res;
...
DWORD topHash(ZKCRYPT_ctx* ctx, DWORD in) {
    DWORD res;
...
DWORD botHash(ZKCRYPT_ctx* ctx, DWORD in) {
    DWORD res;
...
DWORD runEVNN(ZKCRYPT_ctx* ctx, DWORD in, int pos)
/*	16S	&	17S	&	32P	*/
{
    DWORD res;
...
DWORD feedbackStorage(ZKCRYPT_ctx* ctx, DWORD out_ciph)
/* 11P	11S2	34	*/
{
    ...
    if ((ctx->ctrlFeedBack & CTRL_FEEDBACK_RESET) > 0) { /* reset the internal store	*/
        ctx->sttFeedBack = 0;
        ctx->sttST_FeedBack = 0;
        ctx->lclMACstorage = 0;
        lclDisplacedMAC = 0;
    ...
    if ((ctx->ctrlFeedBack & CTRL_FEEDBACK_ENABLE) ==
        0) { /* feedback is disabled return value is 0x00000000*/
        ctx->sttFeedBack = 0x00000000;
        ctx->sttST_FeedBack = 0;
        return 0x00000000;
    }

    if ((ctx->ctrlFeedBack & CTRL_FEEDBACK_MAC) >
        0) { /* MAC feedback takes the input xors it with existing value and return it	*/
    ...
    if ((ctx->ctrlFeedBack & CTRL_FEEDBACK_CIPH) > 0) { /* cipher feedbak enabled	*/
...
void cycleClock(ZKCRYPT_ctx* ctx)
/*	10P	&	10S1	&	10S3	&	*/
{
    BYTE topClockRes = 0;
    BYTE midClockRes = 0;
    ...
    if ((ctx->ctrlClocks & CTRL_CLOCKS_LOAD) > 0) {
        ...
        if ((getBitAtLocation(topClockRes, OUTP_CLOCKS_LFT_SLP) &&
             ...
             getBitAtLocation(botClockRes, OUTP_CLOCKS_RGT_SLP)) == FALSE)
        ...
        if ((getBitAtLocation(topClockRes, OUTP_CLOCKS_RGT_SLP) &&
             ...
             getBitAtLocation(botClockRes, OUTP_CLOCKS_LFT_SLP)) == FALSE)
...
BYTE cycleTopBankClock(ZKCRYPT_ctx* ctx)
/*	13 C	*/
{
    ...
    tmpBit = GETBIT_THREE(ctx->topBankClock_nLFR) ^ GETBIT_ONE(ctx->topBankClock_nLFR) ^
             NFIX(ctx->topBankClock_nLFR, 3) ^
             ((getBitAtLocation(ctx->sttClockFeedBack, CTRL_CLOCKS_FB_Q17)) &
              (GETBIT_ONE(ctx->topBankClockCounter)));
    ...
    if (ctx->topBankClockCounter == 15) {
        ...
        tmpBit = (BYTE)(GETBIT_ONE(tmp_nLFR) ^ GETBIT_TWO(tmp_nLFR) ^
                        (ctx->topBankClock_MC ^
                         (getBitAtLocation(ctx->sttClockFeedBack, CTRL_CLOCKS_FB_JUGG))));
...
BYTE cycleMidBankClock(ZKCRYPT_ctx* ctx)
/* 14 C	*/
{
    BYTE res = 0;
    BYTE tmpBit = 0;
    ...
    tmpBit = GETBIT_FIVE(ctx->midBankClock_nLFR) ^ GETBIT_TWO(ctx->midBankClock_nLFR) ^
             NFIX(ctx->midBankClock_nLFR, 5) ^
             ((getBitAtLocation(ctx->sttClockFeedBack, CTRL_CLOCKS_FB_Q12)) &
              (GETBIT_ONE(ctx->midBankClockCounter)));
...
BYTE cycleBotBankClock(ZKCRYPT_ctx* ctx)
/*	15 C	*/
{
    BYTE res = 0;
    BYTE tmpBit = 0;
    ...
    tmpBit = GETBIT_SIX(ctx->botBankClock_nLFR) ^ GETBIT_ONE(ctx->botBankClock_nLFR) ^
             NFIX(ctx->botBankClock_nLFR, 6) ^
             ((getBitAtLocation(ctx->sttClockFeedBack, CTRL_CLOCKS_FB_Q14)) &
              (GETBIT_ONE(ctx->botBankClockCounter)));
...
void setHashVector(ZKCRYPT_ctx* ctx) {
...
void cycleHashCounter(ZKCRYPT_ctx* ctx)
/* 18 C	*/
{
    ...
    ctx->hashCounter = (~bit27) ^ (getBitAtLocation(ctx->sttClockFeedBack, CTRL_CLOCKS_FB_THSH15) ^
...
void cyclePRandomClock(ZKCRYPT_ctx* ctx, BOOL fClean)
/*	4P	&	4C1B	&	4C2B	*/
{
    ...
    tmpBit = GETBIT_NINE(ctx->longPClock) ^ NFIX(ctx->longPClock, 9) ^
    ...
    /* Short clock */
    /* A8 */
    tmpBit = ~((GETBIT_ONE(ctx->longPClock)) | (GETBIT_TWO(ctx->longPClock)) |
    ...
    F3 = (BYTE)(GETBIT_TWO(tmpLongPClock) ^ GETBIT_FIVE(tmpLongPClock) ^
                GETBIT_EIGHT(tmpLongPClock));

    F4 = (BYTE)(GETBIT_THREE(tmpLongPClock) ^ GETBIT_SIX(tmpLongPClock) ^
                GETBIT_NINE(tmpLongPClock));

    F5 = (BYTE)(GETBIT_ONE(tmpLongPClock) ^ GETBIT_FOUR(tmpLongPClock) ^
...
void ECRYPT_Zkcrypt::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    ZKCRYPT_process_bytes(0, &_ctx, plaintext, ciphertext, msglen);
}
