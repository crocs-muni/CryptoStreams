...
*or later).
*
*Because DECIMv2 is a hardware candidate, we deliberately made a very
...
*
*France Telecom believes that the Decimv2 cipher is covered in part by
 ...
 */
void ECRYPT_Decim::DECIM_process_bytes(
        int action, void* ctxa, const u8* input, u8* output, u32 msglen) {
    DECIM_ctx* ctx = (DECIM_ctx*)ctxa;
    u8 stream_byte;
    ...
    while (bytes_processed < msglen) {
        ...
        if ((ctx->buffer_end == 0) && (bytes_processed < msglen)) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                /*decim_lfsr_filter(ctx);
                decim_lfsr_clock(ctx);
                ctx->buffer_end++; ctx->buffer[ctx->buffer_end]=ctx->bool_out;
                decim_lfsr_filter(ctx);
                decim_lfsr_clock(ctx);
                ctx->buffer_end++; ctx->buffer[ctx->buffer_end]=ctx->bool_out;
                decim_lfsr_filter(ctx);
 ...
 */
void ECRYPT_Decim::DECIM_keystream_bytes(DECIM_ctx* ctx, u8* keystream, u32 length) {
    u8 stream_byte;
    int is_stream_byte = 0;
...
int main(int argc, char** argv) {
    ...
    for (j = 0; j < 32; j++) {
        ...
        DECIM_process_bytes(0, &ctx, message, output, msglen);
...
int main(int argc, char** argv) {
    ...
    ECRYPT_keysetup(&ctx, key, keysize, ivsize);
    ECRYPT_ivsetup(&ctx, iv);
    DECIM_keystream_bytes(&ctx, keystream, msglen);

    return 0;
}
#endif

void ECRYPT_Decim::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    DECIM_process_bytes(0, &_ctx, plaintext, ciphertext, msglen);
}
