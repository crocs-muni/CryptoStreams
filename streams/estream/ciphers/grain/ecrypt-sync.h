...
/* ------------------------------------------------------------------------- */
class ECRYPT_Grain : public estream_interface {
    GRAIN_ctx _ctx;

public:
    ECRYPT_Grain(int rounds)
...
#define GRAIN_GENERATES_KEYSTREAM
    ...
    void GRAIN_keystream_bytes(GRAIN_ctx* ctx,
                               u8* keystream,
...
#define GRAIN_USES_DEFAULT_ALL_IN_ONE /* [edit] */
    ...
    void GRAIN_encrypt_packet(
            GRAIN_ctx* ctx, const u8* iv, const u8* plaintext, u8* ciphertext, u32 msglen);

    void GRAIN_decrypt_packet(
            GRAIN_ctx* ctx, const u8* iv, const u8* ciphertext, u8* plaintext, u32 msglen);
...
#define GRAIN_encrypt_blocks(ctx, plaintext, ciphertext, blocks)                                   \
    ECRYPT_encrypt_bytes(ctx, plaintext, ciphertext, (blocks)*GRAIN_BLOCKLENGTH)

#define GRAIN_decrypt_blocks(ctx, ciphertext, plaintext, blocks)                                   \
    ECRYPT_decrypt_bytes(ctx, ciphertext, plaintext, (blocks)*GRAIN_BLOCKLENGTH)

#ifdef GRAIN_GENERATES_KEYSTREAM

#define GRAIN_keystream_blocks(ctx, keystream, blocks)                                             \
    GRAIN_AE_keystream_bytes(ctx, keystream, (blocks)*GRAIN_BLOCKLENGTH)

#endif

#else

    void GRAIN_encrypt_blocks(GRAIN_ctx* ctx,
                              const u8* plaintext,
                              u8* ciphertext,
                              u32 blocks); /* Message length in blocks. */

    void GRAIN_decrypt_blocks(GRAIN_ctx* ctx,
                              const u8* ciphertext,
                              u8* plaintext,
                              u32 blocks); /* Message length in blocks. */
    ...
    void GRAIN_keystream_blocks(GRAIN_AE_ctx* ctx,
                                const u8* keystream,
