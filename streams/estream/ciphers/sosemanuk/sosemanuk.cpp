 ...
 */
static INLINE unum32 decode32le(unsigned char* data) {
#ifdef __i386
    /*
    ...
    return *(unum32*)data;
#else
    return (unum32)data[0] | ((unum32)data[1] << 8) | ((unum32)data[2] << 16) |
           ((unum32)data[3] << 24);
#endif
}
 ...
 */
static INLINE void encode32le(unsigned char* dst, unum32 val) {
#ifdef __i386__
    /*
...
#else
/* see sosemanuk.h */
void sosemanuk_schedule(sosemanuk_key_context* kc, unsigned char* key, size_t key_len)
#endif
{
    SOSEMANUK_ctx* kc = &_ctx;
/*
...
#define SKS(S, o0, o1, o2, o3, d0, d1, d2, d3)                                                     \
    do {                                                                                           \
...
#define WUP(wi, wi5, wi3, wi1, cc)                                                                 \
    do {                                                                                           \
        unum32 tt = (wi) ^ (wi5) ^ (wi3) ^ (wi1) ^ (0x9E3779B9 ^ (unum32)(cc));                    \
        (wi) = ROTL(tt, 11);                                                                       \
    } while (0)
...
#ifdef SOSEMANUK_ECRYPT
void ECRYPT_Sosemanuk::ECRYPT_ivsetup(const u8* iv)
#else
/* see sosemanuk.h */
...
static void sosemanuk_internal(SOSEMANUK_ctx* rc, const u32* src, u32* dst)
#elif defined SOSEMANUK_SPEED
static unum32 sosemanuk_internal(sosemanuk_run_context* rc, unsigned long counter)
#else
 ...
 * in variable "dd".
 */
#define LRU(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, dd)                                            \
    do {                                                                                           \
        dd = s##x0;                                                                                \
        s##x0 = MUL_A(s##x0) ^ MUL_G(s##x3) ^ s##x9;                                               \
        PLFSR(dd, s##x1, s##x2, s##x3, s##x4, s##x5, s##x6, s##x7, s##x8, s##x9, s##x0);           \
    } while (0)
 ...
 */
#define CC1(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, ee)                                            \
    do {                                                                                           \
        ee = T32(s##x9 + r1) ^ r2;                                                                 \
        PCCVAL(ee);                                                                                \
    } while (0)
...
#define PLFSR(dd, x1, x2, x3, x4, x5, x6, x7, x8, x9, x0)                                          \
    do {                                                                                           \
 ...
 */
static INLINE void
xorbuf(const unsigned char* in1, const unsigned char* in2, unsigned char* out, size_t data_len) {
    unum32 *s1, *s2, *d;
    size_t tl;
    ...
    if (!(((unsigned)in1 | (unsigned)in2 | (unsigned)out) & 0x03)) {
        ...
        case 0x03:
            out[data_len - 3] = in1[data_len - 3] ^ in2[data_len - 3];
        /* fall through */
        case 0x02:
            out[data_len - 2] = in1[data_len - 2] ^ in2[data_len - 2];
        /* fall through */
        case 0x01:
            out[data_len - 1] = in1[data_len - 1] ^ in2[data_len - 1];
        }
    } else {
        while (data_len-- > 0)
            *out++ = *in1++ ^ *in2++;
...
/* see ecrypt-sync.h */
void ECRYPT_Sosemanuk::SOSEMANUK_process_bytes(
        int action, void* ctxa, const u8* input, u8* output, u32 msglen) {
    SOSEMANUK_ctx* ctx = (SOSEMANUK_ctx*)ctxa;
    (void)action;

    while (msglen > 0) {
        u32 ibuf[SOSEMANUK_BLOCKLENGTH / 4];
        u32 obuf[SOSEMANUK_BLOCKLENGTH / 4];
        size_t len;
...
/* see ecrypt-sync.h */
void ECRYPT_Sosemanuk::SOSEMANUK_keystream_bytes(SOSEMANUK_ctx* ctx, u8* keystream, u32 length) {
    ...
    while (length > 0) {
        ...
        if (length >= SOSEMANUK_BLOCKLENGTH) {
            memcpy(keystream, tbuf, SOSEMANUK_BLOCKLENGTH);
            keystream += SOSEMANUK_BLOCKLENGTH;
            length -= SOSEMANUK_BLOCKLENGTH;
        } else {
...
/* see ecrypt-sync.h */
void ECRYPT_Sosemanuk::SOSEMANUK_process_blocks(
        int action, SOSEMANUK_ctx* ctx, const u8* input, u8* output, u32 blocks) {
    (void)action;
...
/* see ecrypt-sync.h */
void ECRYPT_Sosemanuk::SOSEMANUK_keystream_blocks(SOSEMANUK_ctx* ctx, u8* keystream, u32 blocks) {
...
/* see sosemanuk.h */
void sosemanuk_prng(sosemanuk_run_context* rc, unsigned char* out, size_t out_len) {
 ...
 */
static void
maketest(int tvn, unsigned char* key, size_t key_len, unsigned char* iv, size_t iv_len) {
#ifdef SOSEMANUK_ECRYPT
    ECRYPT_ctx ctx;
#else
    sosemanuk_key_context kc;
...
int main(void) {
    ...
    static unsigned char key2[] = {0x00,
                                   0x11,
                                   0x22,
                                   0x33,
                                   0x44,
...
void ECRYPT_Sosemanuk::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    SOSEMANUK_process_bytes(0, &_ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Sosemanuk::ECRYPT_decrypt_bytes(const u8* ciphertext, u8* plaintext, u32 msglen) {
    SOSEMANUK_process_bytes(1, &_ctx, ciphertext, plaintext, msglen);
}
