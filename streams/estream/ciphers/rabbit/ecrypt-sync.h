...
/* ------------------------------------------------------------------------- */
class ECRYPT_Rabbit : public estream_interface {
    RABBIT_ctx _ctx;
...
#define RABBIT_HAS_SINGLE_BYTE_FUNCTION
    ...
    void RABBIT_process_bytes(int action, /* 0 = encrypt; 1 = decrypt; */
                              void* ctx,
                              const u8* input,
                              u8* output,
...
#define RABBIT_GENERATES_KEYSTREAM
    ...
    void RABBIT_keystream_bytes(RABBIT_ctx* ctx,
                                u8* keystream,
...
#define RABBIT_encrypt_packet(ctx, iv, plaintext, ciphertext, mglen)                               \
    RABBIT_process_packet(0, ctx, iv, plaintext, ciphertext, mglen)

#define RABBIT_decrypt_packet(ctx, iv, ciphertext, plaintext, mglen)                               \
    RABBIT_process_packet(1, ctx, iv, ciphertext, plaintext, mglen)

    void RABBIT_process_packet(int action, /* 0 = encrypt; 1 = decrypt; */
                               RABBIT_ctx* ctx,
                               const u8* iv,
                               const u8* input,
                               u8* output,
    ...
    void RABBIT_encrypt_packet(
            RABBIT_ctx* ctx, const u8* iv, const u8* plaintext, u8* ciphertext, u32 msglen);

    void RABBIT_decrypt_packet(
            RABBIT_ctx* ctx, const u8* iv, const u8* ciphertext, u8* plaintext, u32 msglen);
...
#define RABBIT_encrypt_blocks(ctx, plaintext, ciphertext, blocks)                                  \
    ECRYPT_encrypt_bytes(ctx, plaintext, ciphertext, (blocks)*RABBIT_BLOCKLENGTH)

#define RABBIT_decrypt_blocks(ctx, ciphertext, plaintext, blocks)                                  \
    ECRYPT_decrypt_bytes(ctx, ciphertext, plaintext, (blocks)*RABBIT_BLOCKLENGTH)

#ifdef RABBIT_GENERATES_KEYSTREAM

#define RABBIT_keystream_blocks(ctx, keystream, blocks)                                            \
    RABBIT_keystream_bytes(ctx, keystream, (blocks)*RABBIT_BLOCKLENGTH)
...
#define RABBIT_decrypt_blocks(ctx, ciphertext, plaintext, blocks)                                  \
    ...
    void RABBIT_process_blocks(int action, /* 0 = encrypt; 1 = decrypt; */
                               RABBIT_ctx* ctx,
                               const u8* input,
                               u8* output,
    ...
    void RABBIT_encrypt_blocks(RABBIT_ctx* ctx,
                               const u8* plaintext,
                               u8* ciphertext,
                               u32 blocks); /* Message length in blocks. */

    void RABBIT_decrypt_blocks(RABBIT_ctx* ctx,
                               const u8* ciphertext,
                               u8* plaintext,
                               u32 blocks); /* Message length in blocks. */
    ...
    void RABBIT_keystream_blocks(RABBIT_ctx* ctx,
                                 u8* keystream,
