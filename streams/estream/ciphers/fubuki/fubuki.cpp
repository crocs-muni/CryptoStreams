...
static void init_genrand(FUBUKI_ctx* ctx, u32 s);
static void init_by_array(FUBUKI_ctx* ctx, u32 init_key[], int key_length);
static void genrand_whole_array(FUBUKI_ctx* ctx);
static void genrand_tuple_int32(FUBUKI_ctx* ctx, u32 rand_tuple[], s32 len);

static void prepare_multi(FUBUKI_ctx* ctx);
static u32 inv_mod_32(u32 m);
static void prepare_multi_inv(FUBUKI_ctx* ctx);
static void prepare_add_table(FUBUKI_ctx* ctx);
static void crypt_empr(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_empr_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
static void crypt_emer(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_emer_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
static void crypt_emps(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_emps_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
static void crypt_emes(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_emes_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
static void crypt_ma(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_ma_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
static void crypt_mem(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_mem_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
static void crypt_ome(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_ome_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
static void crypt_eme(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_eme_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
static void crypt_vert_rotate(FUBUKI_ctx* ctx, u32 block[Tuple]);
static void crypt_vert_rotate_inv(FUBUKI_ctx* ctx, u32 block[Tuple], u32 param[Tuple]);
...
void hmnencode(FUBUKI_ctx* ctx,
               const u8* plaintext,
               u8* ciphertext,
               u32 msglen,
               int numRounds = Iteration) /* Message length in bytes. */
{
    s32 i, j, repeat;
    u32 msgbuf[Tuple];
    u32 cinpos, coutpos;
    ...
    for (i = 0; i < repeat; i++) {
        ...
        for (j = 0; j < 2 * numRounds;) {
            ...
            switch (c) {
            case 0:
                crypt_empr(ctx, msgbuf);
                break;
            case 1:
                crypt_emer(ctx, msgbuf);
                break;
            case 2:
                crypt_emps(ctx, msgbuf);
                break;
            case 3:
                crypt_emes(ctx, msgbuf);
                break;
            }
            ...
            case 0:
                ...
                break;
            case 1:
                crypt_mem(ctx, msgbuf);
                break;
            case 2:
                crypt_ome(ctx, msgbuf);
                break;
            case 3:
                crypt_eme(ctx, msgbuf);
                break;
            }
            if ((ctx->jump <<= 1) >= Tuple)
                ctx->jump = 1;
               ...
               int numRounds = Iteration) /* Message length in bytes. */
{
    ...
    for (i = 0; i < repeat; i++) {
        ...
        for (k = 0; k < 3 * numRounds; k++)
            genrand_tuple_int32(ctx, temp_rand[k], Tuple);

        ctx->jump = 1 << ((3 * numRounds - 1) % Log_Tuple);

        for (j = 2 * numRounds - 1; j >= 0;) {
            s32 c, t;
            ...
            switch (c) {
            case 0:
                crypt_ma_inv(ctx, msgbuf, temp_rand[--k]);
                break;
            case 1:
                crypt_mem_inv(ctx, msgbuf, temp_rand[--k]);
                break;
            case 2:
                crypt_ome_inv(ctx, msgbuf, temp_rand[--k]);
                break;
            case 3:
                crypt_eme_inv(ctx, msgbuf, temp_rand[--k]);
                break;
            }
            if ((ctx->jump >>= 1) == 0)
                ctx->jump = Tuple / 2;
            ...
            switch (c) {
            case 0:
                crypt_empr_inv(ctx, msgbuf, temp_rand[--k]);
                break;
            case 1:
                crypt_emer_inv(ctx, msgbuf, temp_rand[--k]);
                break;
            case 2:
                crypt_emps_inv(ctx, msgbuf, temp_rand[--k]);
                break;
            case 3:
                crypt_emes_inv(ctx, msgbuf, temp_rand[--k]);
                break;
            }
            if ((ctx->jump >>= 1) == 0)
                ctx->jump = Tuple / 2;
...
void ECRYPT_Fubuki::ECRYPT_init() {}

void ECRYPT_Fubuki::ECRYPT_keysetup(const u8* key,
...
void ECRYPT_Fubuki::ECRYPT_ivsetup(const u8* iv) {
    FUBUKI_ctx* ctx = &_ctx;
    s32 i, j, k, t, s;
    u32 x, init_array[(FUBUKI_MAXKEYSIZE + FUBUKI_MAXIVSIZE) / 32];
...
void ECRYPT_Fubuki::FUBUKI_keystream_bytes(
        FUBUKI_ctx* ctx,
        u8* keystream,
...
void ECRYPT_Fubuki::ECRYPT_encrypt_bytes(const u8* plaintext,
                                         u8* ciphertext,
                                         u32 msglen) /* Message length in bytes. */
{
    FUBUKI_ctx* ctx = &_ctx;
    hmnencode(ctx, plaintext, ciphertext, msglen, _rounds);
}

void ECRYPT_Fubuki::ECRYPT_decrypt_bytes(const u8* ciphertext,
                                         u8* plaintext,
                                         u32 msglen) /* Message length in bytes. */
{
    FUBUKI_ctx* ctx = &_ctx;
    if ((msglen % (4 * Tuple)) != 0) {
        printf("FUBUKI_decrypt_bytes: msglen should be multiple of %d.\n", 4 * Tuple);
        return;
    }
