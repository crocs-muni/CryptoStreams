#include <stdio.h>
#include "primatesv1ape80_parameters.h"
...
void MixColumnsInv(unsigned char *state) //Inverse MixColumns operation on the state
{
    ...
    for(int p=0; p<NumberRows; p++){
        for(int i=0; i<NumberColumns; i++){
            tmp[i]=T18(state[i])^T2(state[NumberColumns*1+i])^T2(state[NumberColumns*2+i])^T18(state[NumberColumns*3+i])^state[NumberColumns*4+i];
