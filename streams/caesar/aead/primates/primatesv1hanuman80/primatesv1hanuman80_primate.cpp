#include <stdio.h>
#include "primatesv1hanuman80_parameters.h"

// CHANGE namespace moved due to includes
namespace Primatesv1hanuman80_raw {

unsigned char  rc_p1[p1rounds];
unsigned char  rc_p4[p4rounds];
...
void GenerateRoundConstants() //Generation of 5-bit round constant by using the primitive polynomial x^5+x^2+1 with initial value 1.
{
    ...
    rc_p4[0] = ((rc_p1[p1rounds-1]<<1) ^ ((rc_p1[p1rounds-1]>>1)&1) ^ ((rc_p1[p1rounds-1]>>4)&1))&31;
    //printf("\nRound constants for p_4: \n %.2x", rc_p4[0]);
    for(int i=1; i<p4rounds; i++){
        rc_p4[i] = ((rc_p4[i-1]<<1) ^ ((rc_p4[i-1]>>1)&1) ^ ((rc_p4[i-1]>>4)&1))&31;
        //printf("\n %.2x", rc_p4[i]);
    }
}
...
void p_4(unsigned char *state)
{
    ...
    {
        primate(state);
        state[RateSizeElements+1]^=rc_p4[rnd];
    }
}
