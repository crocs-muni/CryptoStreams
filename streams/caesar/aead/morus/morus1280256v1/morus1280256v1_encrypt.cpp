...
void inline morus_stateupdate(const uint64_t* msgblk, uint64_t state[][4]) 
{
	uint64_t temp;
...
/*The input to the initialization is the 256-bit key; 128-bit IV;*/
...
inline void morus_enc_aut_step(const uint8_t *plaintextblock,
									  uint8_t *ciphertextblock, uint64_t state[5][4])
{
...
inline void morus_dec_aut_step(uint8_t *plaintextblock,
