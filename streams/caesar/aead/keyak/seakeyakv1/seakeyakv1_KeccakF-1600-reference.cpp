...
#include <stdlib.h>
#include <string.h>
#include "seakeyakv1_brg_endian.h"
#include "seakeyakv1_displayIntermediateValues.h"
#include "seakeyakv1_KeccakF-1600-reference.h"
...
void KeccakF1600_StateExtractAndXORBytesInLane(const void *state, unsigned int lanePosition, unsigned char *data, unsigned int offset, unsigned int length)
{
    ...
    for(i=0; i<length; i++)
        data[i] ^= ((unsigned char *)state)[lanePosition*8+offset+i];
}
