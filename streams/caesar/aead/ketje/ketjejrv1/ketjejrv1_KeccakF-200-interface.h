...
/** Function called at least once before any use of the other KeccakF200_* 
  * functions, possibly to initialize global variables.
  */
void KeccakF200_Initialize( void );

/** Function to initialize the state to the logical value 0^200.
...
/** Function to XOR data given as bytes into the state. 
  ...
  * from @a lanePosition*8 + @a offset*8 
  * to @a lanePosition*8 + @a offset*8 + @a length*8.
...
/** Function to XOR data given as bytes into the state.
  ...
  * to span a whole number of lanes (i.e., multiple of 1 byte).
  * @param  state   Pointer to the state.
  * @param  data    Pointer to the input data.
...
/** Function to sequentially XOR data bytes, apply the Keccak-f[200] 
  * permutation and retrieve data bytes from the state.
  * The bits to modify and to output are restricted to start from the bit 
  * position 0 and  to span a whole number of lanes (i.e., multiple of 1 byte).
