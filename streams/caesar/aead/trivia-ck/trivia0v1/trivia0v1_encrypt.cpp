...
#define CHUNK_SIZE 0x40000000			// Process checksum after 2^30 blocks
...
//--------------------------------//
   ...
   {
        Block = load64(m);
    process_block(Block, Tag, Checksum, SC_State, Z, PROC_BLOCK, PROC_MSG); //Update the Tag and the Checksum
...
// Update the Tag for the Final 4 Checksum Blocks for Message and Get the Key for Variable Length Handling
...
//--------------------------------//
   ...
   while(clen-16 >=8) // Process ALL but the Final Block of the Ciphertext
   {
        ...
        Block = load64(m);
    process_block(Block, Tag, Checksum, SC_State, Z, PROC_BLOCK, PROC_MSG); //Update the Tag in the Decryptor Side
