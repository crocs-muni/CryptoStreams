// 32-bit version of Pi32Cipher256
// implementation of the algorithm with 32-bit registers and security of 256-bits
// bitrate is 256-bits, capacity is 256-bits -> state of the permutation function is 512 bits long
// key is 256 bits, PMN is 128 bits, SMN is 256 bits and the tag is 256 bits
/*
 *
...
#define W 4	      // Length of the bynary words in bits
...
#define ARX32(x0, x1, x2, x3, y0, y1, y2, y3, z0, z1, z2, z3) \
{\
	/*mu-transformation*/ \
	t0 = (0xF0E8E4E2 + x0  + x1 +  x2); \
	t0 = rotl32((t0), 5); \
	t1 = (0xE1D8D4D2 + x0  + x1 +  x3); \
        t1 = rotl32((t1), 11); \
	t2 = (0xD1CCCAC9 + x0 +  x2  + x3); \
	t2 = rotl32((t2), 17); \
	t3 = (0xC6C5C3B8 + x1 +  x2  + x3); \
	t3 = rotl32((t3), 23); \
	\
    t4 = t0 ^ t1 ^ t3; \
    t5 = t0 ^ t1 ^ t2; \
	...
	\
	/*nu-transformation*/ \
    t0 = (0xB4B2B1AC + y0 +  y2  + y3); \
    t0 = rotl32((t0), 3); \
    t1 = (0xAAA9A6A5 + y1 +  y2  + y3); \
    t1 = rotl32((t1), 10); \
    t2 = (0xA39C9A99 + y0  + y1 +  y2); \
    t2 = rotl32((t2), 19); \
    t3 = (0x9695938E + y0  + y1 +  y3); \
    t3 = rotl32((t3), 29); \
	\
    t8  = t1 ^ t2  ^ t3; \
    t9  = t0 ^ t2  ^ t3; \
...
{ \
	ARX32(Constant[8*RoundNumber], Constant[8*RoundNumber+1], Constant[8*RoundNumber+2], Constant[8*RoundNumber+3], \
		  IS[0], IS[1], IS[2], IS[3], \
		  IS[0], IS[1], IS[2], IS[3]); \
	for ( i = 0; i < N - 1; i++ ) { \
			ARX32(IS[4*i], IS[4*i+1], IS[4*i+2], IS[4*i+3], \
	...
	for ( i = N - 1; i >= 1; i-- ) { \
			ARX32(IS[4*(i-1)], IS[4*(i-1)+1], IS[4*(i-1)+2], IS[4*(i-1)+3], \
...
)
{
 ...
 u_int32_t tempTag[CRYPTO_TBYTES/W]={0};
