...
#define ENCRYPT_RND0( A, B, C, D, T0, T1, xkey, r ) \
    ENCRYPT_RND( A,B,C,D,T0,T1,xkey,2*(r)   )

#define ENCRYPT_RND1( A, B, C, D, T0, T1, xkey, r ) \
    ENCRYPT_RND( C,D,A,B,T0,T1,xkey,2*(r)+1 )
 ...
 */
void Twofish::Encrypt(const TwofishKey* aKey,const Twofish_Byte aPlainText[16],Twofish_Byte aCipherText[16]) {
    Encrypt_modified(aKey, aPlainText, aCipherText, 16);
}

void Twofish::Encrypt_modified(const TwofishKey* xkey,const Twofish_Byte p[16],Twofish_Byte c[16], unsigned aRounds)
    {
...
//    ENCRYPT( A,B,C,D,T0,T1,xkey );
    ...
    for (int i = 0; i < aRounds; ++i) {
        if (i % 2 == 0) {
            ENCRYPT_RND0(A, B, C, D, T0, T1, xkey, i / 2);
        } else {
            ENCRYPT_RND1(A, B, C, D, T0, T1, xkey, i / 2);
        }
    }
