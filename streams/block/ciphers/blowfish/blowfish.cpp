...
void blowfish_encrypt(const BYTE in[], BYTE out[], const BLOWFISH_KEY *keystruct, unsigned int rounds)
{
   ...
   l ^= keystruct->p[rounds - 1]; F(l,t); r^= t; //Last iteration has no swap()
   r ^= keystruct->p[rounds];
   l ^= keystruct->p[rounds + 1];
...
void blowfish_decrypt(const BYTE in[], BYTE out[], const BLOWFISH_KEY *keystruct, unsigned int rounds)
{
   ...
   l = (in[0] << 24) | (in[1] << 16) | (in[2] << 8) | (in[3]);
   r = (in[4] << 24) | (in[5] << 16) | (in[6] << 8) | (in[7]);
   for(unsigned int i = rounds + 1; i > 2; --i){
        ITERATION(l,r,t,i);
   }
...
void blowfish_key_setup(const BYTE user_key[], BLOWFISH_KEY *keystruct, size_t len, unsigned int rounds)
{
   ...
   for (idx = 0; idx < 18; idx += 2) {
      blowfish_encrypt(block,block,keystruct,rounds);
