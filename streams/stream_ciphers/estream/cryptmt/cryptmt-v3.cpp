...
static void genrand_bytes_first(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[], u32 len);
static void fast_genrand_bytes_first(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[], u32 len);
static void genrand_bytes(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[], u32 len);
static INLINE void fast_genrand_bytes(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[], u32 len);
static void genrand_block_first(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[]);
static void fast_genrand_block_first(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[]);
static INLINE void genrand_block(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[]);
static INLINE void fast_genrand_block(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[]);
static INLINE void boot_up(CRYPTMT_ctx* ctx, s32 length);
static INLINE void fast_boot_up(CRYPTMT_ctx* ctx, s32 length);
static void filter_16bytes(u32 sfmt[], u32 accum[4], u8 cipher[], const u8 plain[], s32 count);
static INLINE void filter_bytes(u32 sfmt[], u32 accum[4], u8 cipher[], const u8 plain[], s32 len);
static INLINE void booter_am(u32 acc[4], u32 pos1[][4], u32 pos2[][4], s32 count);
...
static void genrand_bytes_first(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[], u32 len) {
    ...
    len = len % 16;
    if (len != 0) {
        filter_bytes(sp->sfmt[i], ctx->accum, &cipher[i * 8], &plain[i * 8], len);
    }
}
...
static void genrand_bytes(CRYPTMT_ctx* ctx, u8 cipher[], const u8 plain[], u32 len) {
    ...
    len = len % 16;
    if (len != 0) {
        filter_bytes(ps->sfmt[i], ctx->accum, &cipher[i * 8], &plain[i * 8], len);
    }
}
...
static INLINE void filter_bytes(u32 sfmt[], u32 accum[4], u8 cipher[], const u8 plain[], int len) {
...
static void boot_up(CRYPTMT_ctx* ctx, s32 length) {
    s32 i, p;
...
void ECRYPT_Cryptmt::ECRYPT_init(void) {
    if (fast_code) {
...
/* Key size in bits. */
/* IV size in bits. */
void ECRYPT_Cryptmt::ECRYPT_keysetup(const u8* key, u32 keysize, u32 ivsize) {
    CRYPTMT_ctx* ctx = &_ctx;
    s32 i;
...
void ECRYPT_Cryptmt::ECRYPT_ivsetup(const u8* iv) {
    CRYPTMT_ctx* ctx = &_ctx;
...
void ECRYPT_Cryptmt::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    CRYPTMT_ctx* ctx = &_ctx;
    if (fast_code) {
        if (ctx->first && (msglen > 0)) {
            if (msglen >= CRYPTMT_BLOCKLENGTH) {
                fast_genrand_block_first(ctx, ciphertext, plaintext);
                ciphertext += CRYPTMT_BLOCKLENGTH;
                plaintext += CRYPTMT_BLOCKLENGTH;
                msglen -= CRYPTMT_BLOCKLENGTH;
                ctx->first = 0;
            } else {
        ...
        while (msglen >= CRYPTMT_BLOCKLENGTH) {
...
/* Message length in bytes. */
void ECRYPT_Cryptmt::ECRYPT_decrypt_bytes(const u8* ciphertext, u8* plaintext, u32 msglen) {
    ECRYPT_encrypt_bytes(ciphertext, plaintext, msglen);
}

/* Message length in blocks. */
void ECRYPT_Cryptmt::CRYPTMT_encrypt_blocks(CRYPTMT_ctx* ctx,
                                            const u8* plaintext,
                                            u8* ciphertext,
                                            u32 blocks) {
    s32 i;
...
/* Message length in blocks. */
void ECRYPT_Cryptmt::CRYPTMT_decrypt_blocks(CRYPTMT_ctx* ctx,
                                            const u8* ciphertext,
                                            u8* plaintext,
                                            u32 blocks) {
