...
u8 iterate(MOUSTIQUE_ctx *s, u8 c_t) 
...
u8 encrypt_bit(MOUSTIQUE_ctx *s, u8 p) 
{
   u8 c;
   c = p ^ iterate(s,0);
   s->C_0[0] = c;
...
u8 decrypt_bit(MOUSTIQUE_ctx *s, u8 c) 
{
   u8 p;
   p = c ^ iterate(s,c);
   return(p);
...
u8 encrypt_byte(MOUSTIQUE_ctx *s, u8 p) 
{
   u8 pbit[8], cbit[8];	
   u16 i;
...
u8 decrypt_byte(MOUSTIQUE_ctx *s, u8 c) 
{
   u8 pbit[8], cbit[8];	
   u16 i;
...
void ECRYPT_Moustique::ECRYPT_ivsetup(
  void* ctxa,
  u8* previous,
  const u8* iv)
{
...
void ECRYPT_Moustique::ECRYPT_encrypt_bytes(
  void* ctxa,     /* no longer constant */
...
void ECRYPT_Moustique::ECRYPT_decrypt_bytes(
  void* ctxa, /* no longer constant */
  const u8* previous,    /* not used */
  const u8* ciphertext, 
  u8* plaintext, 
...
#ifdef MOUSTIQUE_USES_DEFAULT_ALL_IN_ONE
...
void ECRYPT_Moustique::MOUSTIQUE_encrypt_packet(
  MOUSTIQUE_ctx* ctx,
  const u8* iv,
  const u8* plaintext,
  u8* ciphertext,
