...
static void salsa20_wordtobyte(u8 output[64], const u32 input[16], int numRounds) {
    u32 x[16];
    int i;
...
void ECRYPT_Salsa::ECRYPT_init(void) {
    return;
}
...
void ECRYPT_Salsa::ECRYPT_keysetup(const u8* k, u32 kbits, u32 ivbits) {
...
void ECRYPT_Salsa::ECRYPT_ivsetup(const u8* iv) {
    SALSA_ctx* x = &_ctx;
...
void ECRYPT_Salsa::ECRYPT_encrypt_bytes(const u8* m, u8* c, u32 bytes) {
    SALSA_ctx* x = &_ctx;
    u8 output[64];
    int i;
    ...
    if (!bytes)
        return;
    for (;;) {
        salsa20_wordtobyte(output, x->input, _rounds);
...
void ECRYPT_Salsa::ECRYPT_decrypt_bytes(const u8* c, u8* m, u32 bytes) {
    ECRYPT_encrypt_bytes(c, m, bytes);
}
