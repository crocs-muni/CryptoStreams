...
void setup_update(HC128_ctx* ctx) /*each time 16 steps*/
{
    u32 cc, dd;
    cc = ctx->counter1024 & 0x1ff;
    dd = (cc + 16) & 0x1ff;
...
void ECRYPT_HC128::ECRYPT_init(void) {} /* No operation performed */
...
void ECRYPT_HC128::ECRYPT_keysetup(const u8* key,
                                   ...
                                   u32 ivsize)  /* IV size in bits  (128+128*i)*/
{
    ...
    for (i = 0; i < (keysize >> 5); i++)
        ctx->key[i] = U32TO32_LITTLE(((u32*)key)[i]);
...
void ECRYPT_HC128::ECRYPT_ivsetup(const u8* iv) {
    HC128_ctx* ctx = &_ctx;
    u32 i;
    ...
    for (i = 0; i < (ctx->ivsize >> 5); i++)
        ctx->iv[i] = U32TO32_LITTLE(((u32*)iv)[i]);
    ...
    for (i = 16; i < 1024; i++)
        ctx->T[i] =
                f2(ctx->T[i - 2]) + ctx->T[i - 7] + f1(ctx->T[i - 15]) + ctx->T[i - 16] + 256 + i;
...
void ECRYPT_HC128::HC128_process_bytes(int action, /* 0 = encrypt; 1 = decrypt; */
                                       void* ctxa,
                                       const u8* input,
                                       u8* output,
                                       u32 msglen) /* Message length in bytes. */
{
    HC128_ctx* ctx = (HC128_ctx*)ctxa;
    u32 i, keystream[16];
...
void ECRYPT_HC128::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    HC128_process_bytes(0, &_ctx, plaintext, ciphertext, msglen);
}
