  /*
 * DECIM-128 reference implementation.
 *
 * This code is supposed to run on any conforming C implementation (C90
 * or later). 
 *
 * Because DECIM-128 is a hardware candidate, we deliberately made a very 
 ...
 *
 * France Telecom believes that the Decim-128 cipher is covered in part by
...
void ECRYPT_ivsetup(ECRYPT_ctx* ctx, const u8* iv)
{
  ...
  for (i=0;i<288;i++) ctx->lfsr_state[i] = 0 ; 
  ctx->out = 0;
  ctx->bool_out = 0 ;
  ctx->bits_in_byte = 0 ;
  ctx->stream_byte = 0 ;
...
void decim_process_buffer(ECRYPT_ctx *ctx, int *is_stream_byte, u8 *stream_byte)
{
  ...
  ctx->bits_in_byte++ ;
  for(i=0;i<63;i++) ctx->buffer[i]=ctx->buffer[i+1];
  ctx->buffer_end--;
}
...
int main (int argc, char **argv)
{
  ...
  for(j=0;j<32;j++) {    
    ...
    printf("\n\nKey: ");
    for (i=0;i<16;i++) printf("%2.2x ",key[i]);
    
    printf("\nIV : ");
    for (i=0;i<16;i++)  printf("%2.2x ",iv[i]);
...
int main(int argc, char **argv)
{
  ...
  u32 keysize = 128;
  u32 ivsize  = 128;
  static u8 key[] = { 0xf6, 0x3d, 0xb4, 0x85, 0x4f, 0x85, 0x7d, 0xa9, 0x55, 0x43, 0xf6, 0x3d, 0xb4, 0x85, 0x4f, 0x85};
  static u8 iv[]  = { 0x0c, 0x8e, 0xdb, 0x88, 0x01, 0xaa, 0x95, 0xb4, 0x0c, 0x8e, 0xdb, 0x88, 0x01, 0xaa, 0x95, 0xb4};
