 ...
 */
void lfsr_clock(SFINKS_ctx* ctx) {
    u32 newbit;
    u32 i;
    ...
    ctx->LFSR[52]^ctx->LFSR[48]^*/ ctx->LFSR[14] ^
             ctx->LFSR[0];
    for (i = 0; i < 255; i++)
 ...
 */
void sbox_clock(SFINKS_ctx* ctx) {
    u32 i;
    u16 input;
    for (i = 0; i < 6; i++)
        ctx->pipe[i] = ctx->pipe[i + 1];
    for (i = 0; i < 6; i++)
        ctx->linmask[i] = ctx->linmask[i + 1];
 ...
 */
void ECRYPT_Sfinks::ECRYPT_init() {
    GFexp();
    GFpower();
}
...
*/
void ECRYPT_Sfinks::ECRYPT_keysetup(const u8* key,
...
*/
void ECRYPT_Sfinks::ECRYPT_ivsetup(const u8* iv) {
    SFINKS_ctx* ctx = &_ctx;
    u32 i, j, counter;
    u8 ivbyte;
    u16 feedback;
 ...
 */
void ECRYPT_Sfinks::SFINKS_keystream_bytes(SFINKS_ctx* ctx,
                                           u8* keystream,
 ...
 */
void ECRYPT_Sfinks::SFINKS_process_bytes(int action, /* 0 = encrypt; 1 = decrypt; */
                                         void* ctxa,
                                         const u8* input,
                                         u8* output,
                                         u32 msglen) /* Message length in bytes. */
{
    SFINKS_ctx* ctx = (SFINKS_ctx*)ctxa;
    u32 i, j;
    u8 keystreambyte;
    for (i = 0; i < msglen; i++) {
        keystreambyte = 0;
...
void ECRYPT_Sfinks::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    SFINKS_process_bytes(0, &_ctx, plaintext, ciphertext, msglen);
}
