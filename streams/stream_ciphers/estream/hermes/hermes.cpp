/* ----------------------------------------------------------------------------
 *
 * hermes Algorithm   (c) 2006  Dr. Ulrich Kaiser, Germany
 *
 * UKA 08.Nov.2005  extract for API only
 * UKA 06.May.2006  speedup and hardware reduction:
 *                  state nx=17 instead of 37
 *                  stream-rounds=2  instead of 3
 *                  init-rounds=5 instead of 10
 *                  output bytes=8 instead of 16
 *                  set init pointers to zero
 *
 * ----------------------------------------------------------------------------
 */
...
*/
void ECRYPT_Hermes::ECRYPT_init() {}
...
*/
void ECRYPT_Hermes::ECRYPT_keysetup(const u8* key,
...
*/
void ECRYPT_Hermes::ECRYPT_ivsetup(const u8* iv) {
    HERMES_ctx* ctx = &_ctx;
...
*/
void ECRYPT_Hermes::HERMES_process_bytes(int action,
                                         void* ctxa,
                                         const u8* input,
                                         u8* output,
                                         u32 msglen) /* Message length in bytes. */
{
    HERMES_ctx* ctx = (HERMES_ctx*)ctxa;
    /* ----------------- start of algorithm ----------------- */
...
/* ------------------------------------ end -------------------------------- */
void ECRYPT_Hermes::ECRYPT_encrypt_bytes(const u8* plaintext, u8* ciphertext, u32 msglen) {
    HERMES_process_bytes(0, &_ctx, plaintext, ciphertext, msglen);
}
