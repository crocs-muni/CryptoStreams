...
#define SIMD_C32(x)    ((u32)(x))

#define SIMD_HAS_64  1
...
#define SIMD_C32(x)    ((u32)(x ## U))

#else

typedef unsigned long u32;

#define SIMD_C32(x)    ((u32)(x ## UL))
...
/*
 * int should be at least 16 bits wide.
...
#define SIMD_T32(x)    ((x) & SIMD_C32(0xFFFFFFFF))
#define SIMD_ROTL32(x, n)   SIMD_T32(((x) << (n)) | ((x) >> (32 - (n))))
#define SIMD_ROTR32(x, n)   SIMD_ROTL32(x, (32 - (n)))
...
#define simd_rdtsc()                                                         \
  ({                                                                    \
    ...
    __asm__ __volatile__ ("simd_rdtsc" : "=a" (lo), "=d" (hi));              \
    (u64)hi << 32 | lo;                                                 \
  })                                                                    \

#elif defined _MSC_VER && (defined _M_IX86 || defined _M_X64)

#define simd_rdtsc simd___rdtsc

#endif

/* 
 * The SIMD_IS_ALIGNED macro tests if a char* pointer is aligned to an
 * n-bit boundary.
 * It is defined as false on unknown architectures.
 */


#define SIMD_CHECK_ALIGNED(p,n) ((((unsigned char *) (p) - (unsigned char *) NULL) & ((n)-1)) == 0)
