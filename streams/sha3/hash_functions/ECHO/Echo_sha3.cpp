...
/*	Endianess test								*/
	...
	*(unsigned int*)T_o = T;
	if(T_o[0] == 0xAA){
		return ECHO_ET_BIG_ENDIAN;
	}
	if(T_o[0] == 0xDD){
		return ECHO_ET_LITTLE_ENDIAN;
	}
	else{
		return ECHO_ET_MIDDLE_ENDIAN;
	}
}

void Echo::PushString(unsigned long long num, unsigned char* string, unsigned char type){
	unsigned int j;
...
unsigned char Echo::flip_bits(unsigned char c){
	unsigned char res;
...
int Echo::SetSalt(const BitSequence* ECHO_SALT){
	unsigned int topush = *((unsigned int*)ECHO_SALT);
	Echo::PushString(topush, echoState.ECHO_SALT, ECHO_UINT);
	topush = *((unsigned int*)(SALT+4));
	Echo::PushString(topush, echoState.ECHO_SALT+4, ECHO_UINT);
	topush = *((unsigned int*)(SALT+8));
	Echo::PushString(topush, echoState.ECHO_SALT+8, ECHO_UINT);
	topush = *((unsigned int*)(SALT+12));
	Echo::PushString(topush, echoState.ECHO_SALT+12, ECHO_UINT);
...
int Echo::Init(int hashbitlen)
{
	unsigned int i, j;
	unsigned char endianess;

	memset(echoState.state, 0, 16*16);
  	/* set some initial values	    */
  	if(hashbitlen <= 256)
	 	echoState.end_cv = 16;
  	else
	  	echoState.end_cv = 2*16;

  	/* load the IV in the internal state */
 	for(i=0; i< echoState.end_cv; i++){
		if(i%4 == 0){
			Echo::PushString(hashbitlen, (unsigned char*)(echoState.state+i), ECHO_UINT);
		}
		else{
  			echoState.state[i] = 0;
		}
	...
	echoState.hashbitlen = hashbitlen;
	echoState.messbitlen = 0;
	if(hashbitlen <= 256)
		echoState.m_blocksize = 1536;
	else
		echoState.m_blocksize = 1024;
	...
	echoState.padding = 0;
  	...
  	if(hashbitlen <= 256)
		echoState.r = echoNumRoundsLower;
	else
		echoState.r = echoNumRoundsHigher;

#ifdef ECHO_SALT_OPTION
  	/* set the default salt */
  	memset(echoState.ECHO_SALT, 0, 16);
#endif
	
	/*	Endianess testing	   */
	echo_endian = Echo::Endianess();
...
#define echo_abyte(k)  ((k)        & 0xff)
#define echo_bbyte(k) (((k) >>  8) & 0xff)
#define echo_cbyte(k) (((k) >> 16) & 0xff)
#define echo_dbyte(k)  ((k) >> 24)

#define echo_integer(s, i) (*((unsigned int*)(s) + i))

#ifdef ECHO_SALT_OPTION
#define echo_AESround1(s, a, b, c, d) do { \
  a = ECHO_T0[ echo_abyte(echo_integer(s, 0)) ] ^ ECHO_T1[ echo_bbyte(echo_integer(s, 1)) ] ^ ECHO_T2[ echo_cbyte(echo_integer(s, 2)) ] ^ ECHO_T3[ echo_dbyte(echo_integer(s, 3)) ] ^ echo_CNT_r; \
  b = ECHO_T0[ echo_abyte(echo_integer(s, 1)) ] ^ ECHO_T1[ echo_bbyte(echo_integer(s, 2)) ] ^ ECHO_T2[ echo_cbyte(echo_integer(s, 3)) ] ^ ECHO_T3[ echo_dbyte(echo_integer(s, 0)) ] ^ (echo_CNT_r >> 32); \
  c = ECHO_T0[ echo_abyte(echo_integer(s, 2)) ] ^ ECHO_T1[ echo_bbyte(echo_integer(s, 3)) ] ^ ECHO_T2[ echo_cbyte(echo_integer(s, 0)) ] ^ ECHO_T3[ echo_dbyte(echo_integer(s, 1)) ]; \
  d = ECHO_T0[ echo_abyte(echo_integer(s, 3)) ] ^ ECHO_T1[ echo_bbyte(echo_integer(s, 0)) ] ^ ECHO_T2[ echo_cbyte(echo_integer(s, 1)) ] ^ ECHO_T3[ echo_dbyte(echo_integer(s, 2)) ]; \
} while(0);
#define echo_AESround2(s, a, b, c, d) do { \
  echo_integer(s, 0) = ECHO_T0[ echo_abyte(a) ] ^ ECHO_T1[ echo_bbyte(b) ] ^ ECHO_T2[ echo_cbyte(c) ] ^ ECHO_T3[ echo_dbyte(d) ] ^ echo_integer(echoState.ECHO_SALT, 0); \
  echo_integer(s, 1) = ECHO_T0[ echo_abyte(b) ] ^ ECHO_T1[ echo_bbyte(c) ] ^ ECHO_T2[ echo_cbyte(d) ] ^ ECHO_T3[ echo_dbyte(a) ] ^ echo_integer(echoState.ECHO_SALT, 1); \
  echo_integer(s, 2) = ECHO_T0[ echo_abyte(c) ] ^ ECHO_T1[ echo_bbyte(d) ] ^ ECHO_T2[ echo_cbyte(a) ] ^ ECHO_T3[ echo_dbyte(b) ] ^ echo_integer(echoState.ECHO_SALT, 2); \
  echo_integer(s, 3) = ECHO_T0[ echo_abyte(d) ] ^ ECHO_T1[ echo_bbyte(a) ] ^ ECHO_T2[ echo_cbyte(b) ] ^ ECHO_T3[ echo_dbyte(c) ] ^ echo_integer(echoState.ECHO_SALT, 3); \
} while(0);
#else
#define echo_AESround1(s, a, b, c, d) do { \
  a = ECHO_T0[ echo_abyte(echo_integer(s, 0)) ] ^ ECHO_T1[ echo_bbyte(echo_integer(s, 1)) ] ^ ECHO_T2[ echo_cbyte(echo_integer(s, 2)) ] ^ ECHO_T3[ echo_dbyte(echo_integer(s, 3)) ] ^ echo_CNT_r; \
...
#define echo_AESround2(s, a, b, c, d) do { \
  echo_integer(s, 0) = ECHO_T0[ echo_abyte(a) ] ^ ECHO_T1[ echo_bbyte(b) ] ^ ECHO_T2[ echo_cbyte(c) ] ^ ECHO_T3[ echo_dbyte(d) ] ^ ECHO_SALT_A; \
  echo_integer(s, 1) = ECHO_T0[ echo_abyte(b) ] ^ ECHO_T1[ echo_bbyte(c) ] ^ ECHO_T2[ echo_cbyte(d) ] ^ ECHO_T3[ echo_dbyte(a) ] ^ ECHO_SALT_B; \
  echo_integer(s, 2) = ECHO_T0[ echo_abyte(c) ] ^ ECHO_T1[ echo_bbyte(d) ] ^ ECHO_T2[ echo_cbyte(a) ] ^ ECHO_T3[ echo_dbyte(b) ] ^ ECHO_SALT_C; \
  echo_integer(s, 3) = ECHO_T0[ echo_abyte(d) ] ^ ECHO_T1[ echo_bbyte(a) ] ^ ECHO_T2[ echo_cbyte(b) ] ^ ECHO_T3[ echo_dbyte(c) ] ^ ECHO_SALT_D; \
} while(0);
#endif

#define ECHO_WORD_ROUND(s, j, i) do {\
  register unsigned int a, b, c, d;\
  echo_AESround1(s, a, b, c, d);\
  echo_AESround2(s, a, b, c, d);\
  echo_CNT_r++;     \
} while(0);

#define ECHO_MDS(SA, SB, SC, SD) do { \
  unsigned int a, b, c, d, e, f, g;\
  e   = (echo_integer(SA,0) ^ echo_integer(SB,0));\
  f   = (echo_integer(SC,0) ^ echo_integer(SD,0));\
  g   = (echo_integer(SA,0) ^ echo_integer(SD,0));\
  a   = e ^ f;\
  b   =(((e >> 7)&0x01010101)*27)  ^ ((e << 1)&0xfefefefe);\
  c   =(((f >> 7)&0x01010101)*27)  ^ ((f << 1)&0xfefefefe);\
  d   =(((g >> 7)&0x01010101)*27)  ^ ((g << 1)&0xfefefefe);\
  echo_integer(SC,0)   ^= (a ^ c);					\
  echo_integer(SD,0)   ^= (a ^ d);					\
  echo_integer(SA,0)   ^= (a ^ b);					\
  echo_integer(SB,0)   ^= (a ^ b ^ c ^ d);			\
} while(0);


#define ECHO_BIG_SUB_WORDS(echoState) do{\
	ECHO_WORD_ROUND((echoState.state)   , 0, 0);\
	ECHO_WORD_ROUND((echoState.state+4) , 0, 1);\
	ECHO_WORD_ROUND((echoState.state+8) , 0, 2);\
	ECHO_WORD_ROUND((echoState.state+12), 0, 3);\
	ECHO_WORD_ROUND((echoState.state+16), 1, 0);\
	ECHO_WORD_ROUND((echoState.state+20), 1, 1);\
	ECHO_WORD_ROUND((echoState.state+24), 1, 2);\
	ECHO_WORD_ROUND((echoState.state+28), 1, 3);\
	ECHO_WORD_ROUND((echoState.state+32), 2, 0);\
	ECHO_WORD_ROUND((echoState.state+36), 2, 1);\
	ECHO_WORD_ROUND((echoState.state+40), 2, 2);\
	ECHO_WORD_ROUND((echoState.state+44), 2, 3);\
	ECHO_WORD_ROUND((echoState.state+48), 3, 0);\
	ECHO_WORD_ROUND((echoState.state+52), 3, 1);\
	ECHO_WORD_ROUND((echoState.state+56), 3, 2);\
	ECHO_WORD_ROUND((echoState.state+60), 3, 3);\
} while(0);

#define ECHO_BIG_MIX_COLUMN(echoState) do{\
	ECHO_MDS(echoState.state, echoState.state+4, echoState.state+8, echoState.state+12);\
	ECHO_MDS(echoState.state+1, echoState.state+1+4, echoState.state+1+8, echoState.state+1+12);\
	ECHO_MDS(echoState.state+2, echoState.state+2+4, echoState.state+2+8, echoState.state+2+12);\
	ECHO_MDS(echoState.state+3, echoState.state+3+4, echoState.state+3+8, echoState.state+3+12);\
	\
	ECHO_MDS(echoState.state+16, echoState.state+16+4, echoState.state+16+8, echoState.state+16+12);\
	ECHO_MDS(echoState.state+16+1, echoState.state+16+1+4, echoState.state+16+1+8, echoState.state+16+1+12);\
	ECHO_MDS(echoState.state+16+2, echoState.state+16+2+4, echoState.state+16+2+8, echoState.state+16+2+12);\
	ECHO_MDS(echoState.state+16+3, echoState.state+16+3+4, echoState.state+16+3+8, echoState.state+16+3+12);\
	\
	ECHO_MDS(echoState.state+16*2, echoState.state+16*2+4, echoState.state+16*2+8, echoState.state+16*2+12);\
	ECHO_MDS(echoState.state+16*2+1, echoState.state+16*2+1+4, echoState.state+16*2+1+8, echoState.state+16*2+1+12);\
	ECHO_MDS(echoState.state+16*2+2, echoState.state+16*2+2+4, echoState.state+16*2+2+8, echoState.state+16*2+2+12);\
	ECHO_MDS(echoState.state+16*2+3, echoState.state+16*2+3+4, echoState.state+16*2+3+8, echoState.state+16*2+3+12);\
	\
	ECHO_MDS(echoState.state+16*3, echoState.state+16*3+4, echoState.state+16*3+8, echoState.state+16*3+12);\
	ECHO_MDS(echoState.state+16*3+1, echoState.state+16*3+1+4, echoState.state+16*3+1+8, echoState.state+16*3+1+12);\
	ECHO_MDS(echoState.state+16*3+2, echoState.state+16*3+2+4, echoState.state+16*3+2+8, echoState.state+16*3+2+12);\
	ECHO_MDS(echoState.state+16*3+3, echoState.state+16*3+3+4, echoState.state+16*3+3+8, echoState.state+16*3+3+12);\
} while(0);

#define ECHO_WORD_SWAP(echoState, a, b) do { \
  unsigned int aa, bb;\
  aa = *(echoState.state+4*a);   *(echoState.state+4*a)   = *(echoState.state+4*b);    *(echoState.state+4*b)   = aa;\
  bb = *(echoState.state+4*a+1); *(echoState.state+4*a+1) = *(echoState.state+4*b+1);  *(echoState.state+4*b+1) = bb;\
  aa = *(echoState.state+4*a+2); *(echoState.state+4*a+2) = *(echoState.state+4*b+2);  *(echoState.state+4*b+2) = aa;\
  bb = *(echoState.state+4*a+3); *(echoState.state+4*a+3) = *(echoState.state+4*b+3);  *(echoState.state+4*b+3) = bb;\
} while(0); 

#define ECHO_BIG_SHIFT_ROWS(echoState) do {\
	ECHO_WORD_SWAP(echoState, 9,  13);\
	ECHO_WORD_SWAP(echoState, 5,  13);\
	ECHO_WORD_SWAP(echoState, 1,  13);\
	\
	ECHO_WORD_SWAP(echoState, 2,  10);\
	ECHO_WORD_SWAP(echoState, 6,  14);\
	\
	ECHO_WORD_SWAP(echoState, 3,  15);\
	ECHO_WORD_SWAP(echoState, 7,  15);\
	ECHO_WORD_SWAP(echoState, 11, 15);\
} while(0);\


#define ECHO_LOADmessage(echoState, data) do {\
	if(echoState.hashbitlen <= 256){\
		memcpy(echoState.state+16, data, 1536 >> 3);\
	}\
	else{\
		memcpy(echoState.state+2*16, data, 1024 >> 3);\
	}\
}while(0);

#define ECHO_XOR_MEM(A, B, size) do { \
  	unsigned int i;\
  	for(i=0; i<size; i++)\
		((unsigned int*)A)[i] ^=  ((unsigned int*)B)[i];\
} while(0); 

#define ECHO_FEEDforward(echoState, oldcv) do{\
	if(echoState.end_cv == 16){\
		ECHO_XOR_MEM(echoState.state, 			echoState.state + echoState.end_cv, 	echoState.end_cv);\
		ECHO_XOR_MEM(echoState.state + 2*echoState.end_cv, echoState.state + 3*echoState.end_cv, echoState.end_cv);\
		ECHO_XOR_MEM(echoState.state, 			echoState.state + 2*echoState.end_cv, echoState.end_cv);\
		ECHO_XOR_MEM(echoState.state, 			oldcv, 				echoState.end_cv);\
	}\
	else{\
		ECHO_XOR_MEM(echoState.state, echoState.state + echoState.end_cv, echoState.end_cv);\
		ECHO_XOR_MEM(echoState.state, oldcv, echoState.end_cv);\
...
#define ECHO_SAVEcv(echoState, oldcv) do {\
	memcpy(oldcv, &echoState, (echoState.end_cv)*sizeof(int)); \
	if(echoState.end_cv == 16){\
		ECHO_XOR_MEM(oldcv, echoState.state + echoState.end_cv, 	echoState.end_cv);\
		ECHO_XOR_MEM(oldcv, echoState.state + 2*echoState.end_cv, 	echoState.end_cv);\
		ECHO_XOR_MEM(oldcv, echoState.state + 3*echoState.end_cv, 	echoState.end_cv);\
	}\
	else{\
		ECHO_XOR_MEM(oldcv, echoState.state + echoState.end_cv, 	echoState.end_cv);\
	}\
...
#define echo_reverse_state(state) do {\
	unsigned int i;\
	for(i=0; i<4*16; i++){\
		echo_int_revert (echo_integer(state, i));\
	}\
} while(0);
...
{ 	
	...
	ECHO_LOADmessage(echoState, echoState.data);

	/*	Preparing the Feedforward operation					*/
	ECHO_SAVEcv(echoState, oldcv);
	
	echo_r = echoState.r;	
	echoState.CNT += (1-echoState.padding) * echoState.m_blocksize + echoState.padding * echoState.messbitlen;

	echo_CNT_r = echoState.CNT;

	if(echo_endian == ECHO_ET_BIG_ENDIAN){	
		echo_reverse_state(echoState.state);
	}

	do {
	/*	Applying 2 rounds AES on each word					*/

		ECHO_BIG_SUB_WORDS(echoState);

	/*	Shift rows								*/

		ECHO_BIG_SHIFT_ROWS(echoState);

	/*	Mix columns								*/
		ECHO_BIG_MIX_COLUMN(echoState);
	...
	/*	Feedforwarding 								*/
	ECHO_FEEDforward(echoState, oldcv);

	return SUCCESS;
}

int Echo::Update(const BitSequence *data, DataLength databitlen){
	...
	if(echoState.messbitlen % 8 != 0){
		return UPDATE_WBITS_TWICE;
	}
	/*	Take the input bits and concatenate to the existing data				*/
	n_blocks = (databitlen + echoState.messbitlen)/echoState.m_blocksize;

	/*	Process if enough data									*/
	for(i=0; i<n_blocks; i++){
		if(i == 0)
			memcpy(echoState.data + (echoState.messbitlen >> 3), data, (echoState.m_blocksize-echoState.messbitlen) >> 3);
		else
			memcpy(echoState.data, data + (i-1)*(echoState.m_blocksize >> 3) + ((echoState.m_blocksize-echoState.messbitlen) >> 3), echoState.m_blocksize >> 3);
	/*	Update the internal state and feedforward						*/
		Echo::Compress();
	}

	/*	Store the remaining bits for the next update						*/
	if(((databitlen + echoState.messbitlen) % echoState.m_blocksize) != 0){
		if(n_blocks >= 1){
			unsigned int remaining_bits = ((databitlen + echoState.messbitlen) % echoState.m_blocksize);
			memcpy(echoState.data, data + (n_blocks-1)*(echoState.m_blocksize >> 3) + ((echoState.m_blocksize-echoState.messbitlen) >> 3), remaining_bits >> 3);
			echoState.messbitlen = remaining_bits;
			/*	Treat the last bits							*/
			if(databitlen % 8 != 0)
				echoState.data[(echoState.messbitlen >> 3)] = Echo::flip_bits(Echo::flip_bits(data[(databitlen >> 3)]) & (0xff >> (8-(databitlen%8))));
		}
	/*	If there was not enough data, concatenate and wait for the next update			*/
	/*	or final										*/
		else{
			memcpy(echoState.data + (echoState.messbitlen >> 3), data, databitlen >> 3);
			echoState.messbitlen += databitlen;
			/*	Treat the last bits							*/
			if(databitlen % 8 != 0)
				echoState.data[(echoState.messbitlen >> 3)] = Echo::flip_bits(Echo::flip_bits(data[(databitlen >> 3)]) & (0xff >> (8-(databitlen%8))));
		}
...
int Echo::Final(BitSequence *hashval)
{
  	unsigned int i;
	...
	for(i=0; i<(echoState.hashbitlen >> 3); i++)
  		hashval[i] = 0;
  	/* Then, we pad the message											*/ 
  	if(echoState.messbitlen <= (echoState.m_blocksize-145)){
  	/*	We can perform a one block padding									*/
 	/*	We add a 1 bit 												*/
		unsigned char c = Echo::flip_bits(echoState.data[echoState.messbitlen >> 3]);
	/*	Enough room for two bits in the last byte ?								*/
		if((echoState.messbitlen%8) != 0){
			c &= ~(0xff << (echoState.messbitlen%8));
			echoState.data[echoState.messbitlen >> 3] = Echo::flip_bits(c ^ (0x1 << (echoState.messbitlen % 8)));
		}
	/*	We pad the remaining bits										*/	
		memset(echoState.data + (echoState.messbitlen >> 3) + 1, 0, (echoState.m_blocksize >> 3)- (144 >> 3) - (echoState.messbitlen >> 3) - 1 ); 
		if((echoState.messbitlen%8) == 0)
			echoState.data[(echoState.messbitlen >> 3)] = 0x80;
	/*	Prepare the message length and the hash length								*/	
		Echo::PushString(echoState.hashbitlen, (unsigned char*)(echoState.data + (echoState.m_blocksize >> 3) - (144 >> 3)), ECHO_USINT);
		Echo::PushString(echoState.CNT + echoState.messbitlen, (unsigned char*)(echoState.data + (echoState.m_blocksize >> 3) - (144 >> 3) + 2), ECHO_ULL);
		Echo::PushString(0, (unsigned char*)(echoState.data + (echoState.m_blocksize >> 3) - (144 >> 3) + 10), ECHO_ULL);
						
	/*	We finalize the compression on the last block								*/	
		if((echoState.messbitlen%echoState.m_blocksize) == 0){
			echoState.messbitlen = 0;
			echoState.CNT = 0;
		}
		echoState.padding = 1;
  	...
  	/*	Not enough room for one block : two blocks padding							*/
		...
		memset(echoState.data + (echoState.messbitlen >> 3) + 1, 0, (echoState.m_blocksize >> 3) - (echoState.messbitlen >> 3) - 1); 	
		if((echoState.messbitlen%8) == 0)
			echoState.data[(echoState.messbitlen >> 3)] = 0x80;
	...
	/*	Prepare the message length and the hash length								*/		
		memset(echoState.data, 0, (echoState.m_blocksize >> 3)); 	
		Echo::PushString(echoState.hashbitlen, (unsigned char*)(echoState.data + (echoState.m_blocksize >> 3) - (144 >> 3)), ECHO_USINT);
		Echo::PushString(echoState.CNT, (unsigned char*)(echoState.data + (echoState.m_blocksize >> 3) - (144 >> 3) + 2), ECHO_ULL);
		Echo::PushString(0, (unsigned char*)(echoState.data + (echoState.m_blocksize >> 3) - (144 >> 3) + 10), ECHO_ULL);
	...
	if(echoState.hashbitlen%8 == 0){
		memcpy(hashval, echoState.state, (echoState.hashbitlen)/8);  	
	}
	else{
		((unsigned char*)echoState.state)[((echoState.hashbitlen)/8)] = \
		((unsigned char*)echoState.state)[((echoState.hashbitlen)/8)] & (0xff << (8-(echoState.hashbitlen%8)));	
		memcpy(hashval, echoState.state, ((echoState.hashbitlen)/8) + 1);  			
	}
...
int Echo::Hash(int hashbitlen, const BitSequence *data, DataLength databitlen, BitSequence *hashval) 
{ 
  //HashReturn S;
  int S;
  //hashState state;
  S = Echo::Init(hashbitlen); 
  if(S != SUCCESS) return S; 
  S = Echo::Update(data, databitlen); 
  if(S != SUCCESS) return S; 
  return Echo::Final(hashval); 
}

Echo::Echo(const int numRounds) {
	if (numRounds == -1) {
		echoNumRoundsLower = ECHO_NUM_ROUNDS_LOWER;
		echoNumRoundsHigher = ECHO_NUM_ROUNDS_HIGHER;
	} else {
		echoNumRoundsLower = numRounds;
		echoNumRoundsHigher = numRounds;
	}
}